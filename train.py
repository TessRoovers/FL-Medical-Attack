...
def train_model(
    ...
    # 1. Create dataset
    try:
        dataset = CarvanaDataset(dir_img, dir_mask, img_scale=img_scale)
    except (AssertionError, RuntimeError, IndexError):
        dataset = BasicDataset(dir_img, dir_mask, img_scale=img_scale)

    # 2. Split into train / validation / test partitions -- Li et al. (2022)
    n_train = 180
    n_val = 20
    n_test = 47
...
if __name__ == '__main__':
    ...
    try:
        torch.save(model.state_dict(), './models/simple_segmentation.pth')
    except:
